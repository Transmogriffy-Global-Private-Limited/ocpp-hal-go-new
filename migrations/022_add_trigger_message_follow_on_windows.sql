...
CREATE TABLE v1_trigger_message_follow_on_windows (
    ...
    CHECK ((state = 'OPEN' AND observed_at IS NULL AND closed_at IS NULL) OR (state = 'OBSERVED' AND observed_at IS NOT NULL) OR (state = 'CLOSED' AND closed_at IS NOT NULL))
);
-- Bounds each inbound OCPP handler lookup to windows that can still contain
-- its observation, rather than scanning historical charger operations.
