...
| Transaction persistence | ADAPTED | V1 PostgreSQL commands, credentials, transactions, stop workflows, and facts own new-runtime truth. Exact OCPP IDs remain durable and only validated StopTransaction evidence completes them. Concurrent STOP lifecycle mutations lock transaction, workflow, commands, then facts; bounded retry covers only locally aborted PostgreSQL work. |
